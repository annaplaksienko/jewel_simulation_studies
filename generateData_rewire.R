...
#'
#' @importFrom igraph barabasi.game as_adjacency_matrix graph_from_adjacency_matrix plot.igraph intersection
...
generateData_rewire <- function (K, n, p,
  ...
  G_list[[1]] <- barabasi.game(n = p, power = power, m = m,
                          directed = FALSE)
  if (makePlot == TRUE) plot.igraph(G_list[[1]], 
                                    vertex.size = 3,
                                    vertex.label = NA)
